...
function isOnSingleSide(path) {
  return (
    path.every((point) => point[0] === path[0][0]) ||
    path.every((point) => point[1] === path[0][1])
  );
}
...
function isAdjacentAngle(point, prevPoint, nextPoint, type) {
  ...
  return (
    (prevPoint[coord] > point[coord] && nextPoint[coord] > point[coord]) ||
    (prevPoint[coord] < point[coord] && nextPoint[coord] < point[coord])
  );
}
...
function splitSquareSide2(minX, maxX, minY, maxY, splitPoint) {
  if (splitPoint[0] === minX && splitPoint[1] < maxY) {
    return "left";
  }
  if (splitPoint[0] === maxX && splitPoint[1] > minY) {
    return "right";
  }
  if (splitPoint[1] === minY && splitPoint[0] > minX) {
    return "bottom";
  }
  if (splitPoint[1] === maxY && splitPoint[0] < maxX) {
    return "top";
  }
}
...
function adjacentPathReference(point, cloudPoint, commonCoord) {
  ...
  mapFrom(cloudPoint, originIdx, (pt, idx) => {
    if (
      idx - minIdx === 1 &&
      point[commonCoord] === cloudPoint[minIdx][commonCoord]
    ) {
      minIdx = idx;
      path.splice(0, 0, pt);
    }
  ...
  const prevPoint =
    cloudPoint[minIdx === 0 ? cloudPoint.length - 1 : minIdx - 1];
  const nextPoint =
    cloudPoint[maxIdx === cloudPoint.length - 1 ? 0 : maxIdx + 1];
  path.splice(0, 0, prevPoint);
  path.push(nextPoint);
  return path;
}

function isInAdjacentPath(
  point,
  prevPoint,
  nextPoint,
  pointCloud,
  ...
  return (
    (point[progressCoord] > adjPathRef[0][progressCoord] &&
      point[progressCoord] >
        adjPathRef[adjPathRef.length - 1][progressCoord]) ||
    (point[progressCoord] < adjPathRef[0][progressCoord] &&
      point[progressCoord] < adjPathRef[adjPathRef.length - 1][progressCoord])
  );
}

function isFirstPointInCrossingAdjacentPath(
  point,
  prevPoint,
  nextPoint,
  pointCloud,
...
function isPointStrictlyInMiddle(
  startPoint,
  endPoint,
  point,
  commonCoord,
  ...
  return pointCloud
    .filter((point, idx) => {
      ...
      if (arePointsEqual(endPoint, point)) {
        const prevPoint =
          pointCloud[idx === 0 ? pointCloud.length - 1 : idx - 1];
        const nextPoint =
          pointCloud[idx === pointCloud.length - 1 ? 0 : idx + 1];
        if (
      ...
      return isPointStrictlyInMiddle(
        startPoint,
        endPoint,
        point,
        commonCoord,
    ...
    .filter((point) => {
      ...
      return (
        (!isAdjacentAngle(
          ...
          prevPoint,
          nextPoint,
          commonCoord === 0 ? "vertical" : "horizontal"
        ) &&
          !isInAdjacentPath(
            point,
            prevPoint,
        ...
        isFirstPointInCrossingAdjacentPath(
          point,
          prevPoint,
          nextPoint,
          pointCloud,
...
function isInnerCorner(minX, maxX, minY, maxY, point, polygonPoints) {
  ...
  if (arePointsEqual([minX, minY], point)) {
    return (
      crossPointNb(topRef, point, polygonPoints) % 2 === 1 ||
      crossPointNb(rightRef, point, polygonPoints) % 2 === 1
    );
  }
  if (arePointsEqual([minX, maxY], point)) {
    return (
      crossPointNb(bottomRef, point, polygonPoints) % 2 === 1 ||
      crossPointNb(rightRef, point, polygonPoints) % 2 === 1
    );
  }
